...
from transition_model.KinematicBicycleModel.kinematic_model import \
    KinematicBicycleModel
from transition_model.predmlp import TrajPredGaussion, TrajPredMLP
...
class DCP_Agent():
    ...
    def act(self, candidate_trajectories_tuple, dynamic_map):
        ...
        obs = self.calculate_state(dynamic_map)
    ...
    def generate_imagined_trajectories(self, obs, candidate_trajectories_tuple, fixed_ego_traj=True):
        if fixed_ego_traj:
            ...
            for ensemble_index in range(self.ensemble_num):
                ego_trajectory, rollout_trajectory = self.ensemble_transition_model.rollout_jit_acc(obs, action, ensemble_index)
